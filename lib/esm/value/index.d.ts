...
declare type MaybeValueRemoveOr<Model, Key extends keyof Model, ValueType> = Partial<Pick<Model, Key>> extends Pick<Model, Key> ? ValueRemove | ValueType : ValueType;
declare type MaybeValueRemove<Model, Key extends keyof Model> = Partial<Pick<Model, Key>> extends Pick<Model, Key> ? ValueRemove : Undefined<Model[Key]> extends Model[Key] ? ValueRemove : never;
declare type Undefined<T> = T extends undefined ? T : never;
/**
 * The value types to use for update operation.
 */
export declare type UpdateValue<Model, Key> = Key extends keyof Model ? Model[Key] extends infer Type ? Type extends number ? MaybeValueRemoveOr<Model, Key, ValueIncrement> : Type extends Array<any> ? MaybeValueRemoveOr<Model, Key, ValueArrayUnion | ValueArrayRemove> : Type extends Date ? MaybeValueRemoveOr<Model, Key, ValueServerDate> : MaybeValueRemove<Model, Key> : never : never;
/**
 * The value types to use for add operation.
 */
export declare type AddValue<T> = T extends ServerDate ? ValueServerDate : never;
/**
 * The value types to use for set operation.
 */
export declare type SetValue<Type> = Type extends ServerDate ? ValueServerDate : never;
/**
 * The value types to use for upset operation.
 */
export declare type UpsetValue<T> = T extends number ? ValueIncrement : T extends Array<any> ? ValueArrayUnion | ValueArrayRemove : T extends ServerDate ? ValueServerDate : never;
